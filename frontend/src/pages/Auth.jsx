...
export default function Auth() {
  ...
  // ── FIX: mobile only required for signup, not login ──
  const validate = () => {
    const newErrors = {};
    ...
    // mobile only required for signup
    if (!isLogin) {
  ...
  // ── After getting token — set user and navigate ─────
  const finalizeLogin = (data) => {
    resetStore();
    localStorage.removeItem("token");

    localStorage.setItem("token", data.token);
    setToken(data.token);        // memory store
    setStoredUser(data.user);
    setUser(data.user);

    const route = REDIRECT_MAP[data.redirectTo] || "/path-selection";
    navigate(route);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Verified user — token returned directly, skip OTP
      if (data.requiresOtp === false && data.token) {
        finalizeLogin(data);
        return;
      }

      // ⚠️ New / unverified user — show OTP screen
      if (data.userId) {
  ...
  if (otpStep) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-white to-purple-100 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 px-4">
        <motion.div
        ...
        >
          ...
          <input
            ...
            placeholder="• • • • • •"
            className="w-full p-4 text-center text-3xl tracking-[1rem] rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100 outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 mb-3 transition-all"
          />

          {otpError && (
            <p className={`text-sm mb-3 ${otpError.startsWith("✅") ? "text-green-500" : "text-red-500 dark:text-red-400"}`}>
              {otpError}
            </p>
          )}
          ...
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none mb-3"
          >
            {otpLoading ? "Verifying..." : "Verify OTP"}
          </button>

          <button onClick={handleResend} className="text-purple-500 dark:text-purple-400 text-sm hover:underline">
            Didn't receive? Resend OTP
          </button>

          <button
            onClick={() => { setOtpStep(false); setOtp(""); setOtpError(""); }}
            className="block w-full mt-3 text-gray-400 text-sm hover:text-gray-600 transition"
          >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none"
            >
