...
export default function DayPage() {
  ...
  const allTasksDone =
    tasks.length > 0 && tasks.every((t) => doneTasks.includes(String(t.id)));
            ...
            if (event.data === window.YT.PlayerState.PLAYING) {
              const interval = setInterval(() => {
                ...
                if (t >= activeTask.end) {
                  ...
                  // ✅ SAVE TO BACKEND
                  handleTaskCompletion(dayNum, String(activeTask.id));
  ...
  const handleTaskCompletion = async (dayNum, taskId) => {
    try {
      // Update local store
      addCompletedTask(dayNum, taskId);
    ...
    if (option === currentQuiz.answer) {
      ...
      setTimeout(() => {
        if (currentQuizIndex < quizData.length - 1) {
          ...
          // ✅ All quiz questions done - mark day complete
          handleDayCompletion();
        }
      }, 800);
    } else {
  ...
  const handleDayCompletion = async () => {
    try {
      ...
      const quizCorrect = quizData.filter(
        (q, i) =>
          i < currentQuizIndex ||
          (i === currentQuizIndex && selectedOption === q.answer)
      ).length;
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 shadow text-center mt-6 border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-gray-500 mb-6">
              You already finished this day. Keep going!
            </p>
            ...
            {!quizStarted ? (
              <div className="text-center py-6">
                ...
                <p className="text-gray-500 mb-1 text-sm">
                  Take the quiz to complete Day {dayNum}
                </p>
                <p className="text-purple-500 font-semibold text-sm mb-5">
                  +{quizXP} XP available
                </p>
                <button
              ...
              <>
                ...
                <div className="space-y-2">
                  {currentQuiz.options.map((option) => {
                    let style =
                      "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-purple-300 hover:bg-purple-50 dark:hover:bg-purple-950";
                    if (selectedOption) {
                      if (option === currentQuiz.answer)
                        style =
                          "border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-300";
                      else if (
                        option === selectedOption &&
                        quizResult === "wrong"
                      )
                        style =
                          "border-red-400 bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-300";
                    }
                    return (
                      <button
                        key={option}
                ...
                {quizResult === "wrong" && (
                  <div className="mt-4 text-center">
                    ...
                    <button
                      onClick={retryWrong}
                      className="text-sm px-4 py-2 bg-red-50 border border-red-200 text-red-600 rounded-lg hover:bg-red-100"
                    >
                      Retry
                    </button>
                  </div>
        ...
        {dayDone && (
          <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-8 text-white text-center shadow-xl">
            ...
            <p className="text-purple-200 mb-6 text-sm">
              {saving ? "Saving progress..." : "Streak updated. Keep it going!"}
            </p>
            <div className="flex gap-3 justify-center">
              <button
                onClick={() => navigate("/dashboard")}
                className="px-5 py-2 bg-white/20 rounded-lg hover:bg-white/30 text-sm"
              >
                Dashboard
              </button>
              {dayNum < totalDays && (
                <button
                  onClick={goNext}
                  className="px-5 py-2 bg-white text-purple-700 font-semibold rounded-lg hover:bg-purple-50 text-sm"
                >
                  Next Day →
                </button>
              )}
...
function BackBtn({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition"
    >
      ← Back
    </button>
  );
